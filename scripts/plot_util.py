...
class PlotHelper:
    ...
    # --------------------------------------------------------------------------
    def plot_fig_legend(self, legend_width=0.92, legend_height=0.2, ncol=5, \
        font_size=13, legend_start=0.95, columnspacing=None):
        '''
        draw the legend of figure
        '''
        bbox = (0.5-legend_width/2.0, legend_start-legend_height, legend_width, 
            legend_height)
        
        self.plt.figlegend(fontsize=font_size, loc='upper center', 
            bbox_to_anchor=bbox, mode="expand", borderaxespad=0.01, 
            ncol=ncol, columnspacing=columnspacing)

    # --------------------------------------------------------------------------
    def plot_subplots_adjust(self, left_space=0.95, bottom_space=0.1, \
        top_space=0.9, right_space=0.25, wspace=0.25, hspace=0.3):
        '''
    ...
    def plot_and_save(self, folder, name):
        ...
        self.plt.savefig('%s.png' % filename, format='png', dpi=1000, bbox_inches="tight")
        self.plt.savefig('%s.eps' % filename, format='eps', dpi=1000, bbox_inches="tight")
        self.plt.savefig('%s.pdf' % filename, format='pdf', dpi=1000, bbox_inches="tight")
        self.plt.show()
